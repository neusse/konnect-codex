---
name: konnect
description: "Protect KiCad project integrity through Konnect MCP tools. Use for any task involving KiCad, schematics, PCBs, libraries, manufacturing outputs, or .kicad_* files."
---
...
|---|---|---|
| "Review my schematic" | 2 | Use the visible schematic-analysis tools |
| "Change R5 from 10k to 4.7k" | 1 | `edit_schematic_component` |
| "What's connected to SCL?" | 2 | `get_net_connections` |
| "Add a 100nF cap to U3 VCC" | 1 | Use the visible component and wiring tools |
| "Rename net /CLK to /SYS_CLK" | 1 | Warn about downstream effects, then MCP tools |
| "Run DRC" | 1 | `run_drc` |
| "Export Gerbers" | 1 | `export_gerber` |
| "Just patch line 247 of the .kicad_sch" | REFUSE | Explain risks, offer MCP alternative |
| "Add ESD protection to USB lines" | 1 | Use the visible component and wiring tools |
| "Check if board is ready for fab" | 2 | Use verification and design-review tools |
...
catalogue rather than relying on a count written here.
...
```
1. get_project_info                        → inspect the project
2. add_schematic_component (repeat)        → place parts
3. connect_pins / add_wire / add_schematic_net_label → wire the circuit
4. run_erc / run_design_review             → validate the design
```
